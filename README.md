...
Docker is used only for PostgreSQL + PostGIS during development. The FastAPI
backend and React frontend run locally on Windows.
...
The database is available at `localhost:5432` with database `geoai_db`, user
`geoai`, and password `geoai`.
...
The local backend uses `DATABASE_URL=postgresql+psycopg2://geoai:geoai@localhost:5432/geoai_db`.

### 4. Start FastAPI locally

```powershell
$env:DATABASE_URL = "postgresql+psycopg2://geoai:geoai@localhost:5432/geoai_db"
$env:PYTHONPATH = "$PWD\agent;$PWD\rag"
